  ...
  *neighbors(i) {
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      if (y06 <= this.ymin)
        return null;
      if ((c = (this.ymin - y06) / vy) < t)
        y = this.ymin, x = x06 + (t = c) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax)
        return null;
      if ((c = (this.ymax - y06) / vy) < t)
        y = this.ymax, x = x06 + (t = c) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax)
        return null;
      if ((c = (this.xmax - x06) / vx) < t)
        x = this.xmax, y = y06 + (t = c) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin)
        return null;
      if ((c = (this.xmin - x06) / vx) < t)
        x = this.xmin, y = y06 + (t = c) * vy;
    }
    return [x, y];
  }
  _edgecode(x, y) {
...
function collinear(d) {
  ...
  for (let i = 0; i < triangles.length; i += 3) {
    const a = 2 * triangles[i], b = 2 * triangles[i + 1], c = 2 * triangles[i + 2], cross2 = (coords[c] - coords[a]) * (coords[b + 1] - coords[a + 1]) - (coords[b] - coords[a]) * (coords[c + 1] - coords[a + 1]);
    if (cross2 > 1e-10)
      return false;
  }
  return true;
...
function circle_default() {
  var center = constant_default2([0, 0]), radius = constant_default2(90), precision = constant_default2(6), ring, rotate, stream = { point: point2 };
  function point2(x, y) {
...
function circle_default2(radius) {
  var cr = cos(radius), delta = 6 * radians2, smallRadius = cr > 0, notHemisphere = abs(cr) > epsilon4;
  ...
  function intersect(a, b, two) {
    ...
    if (!determinant)
      return !two && a;
    var c1 = cr * n2n2 / determinant, c2 = -cr * n1n2 / determinant, n1xn2 = cartesianCross(n1, n2), A5 = cartesianScale(n1, c1), B3 = cartesianScale(n2, c2);
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = sqrt(a * a + b * b + c * c), phi2 = asin(c /= m), lambda22 = abs(abs(c) - 1) < epsilon4 || abs(lambda04 - lambda12) < epsilon4 ? (lambda04 + lambda12) / 2 : atan2(b, a), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
...
function symlogish(transform) {
  var c = 1, scale2 = transform(transformSymlog(c), transformSymexp(c));
...
//# sourceMappingURL=chunk-OUO3LTTW.js.map
